...
<form action="<%= request.getContextPath() %>/admin/member/update.do" method="post">
    <input type="hidden" name="menu" value="adminMember"/>
    ...
    <div style="display: flex; justify-content: center; margin-top: 20px;">
        <div style="display: flex; align-items: center;">
            <% if (currentPage > 1) { %>
                <a href="?menu=adminMember&page=<%= currentPage - 1 %>" style="margin: 0 5px; padding: 5px 10px; border: 1px solid #ccc; text-decoration: none; border-radius: 3px;">이전</a>
            <% } else { %>
            ...
            %>
                ...
                <% } else { %>
                    <a href="?menu=adminMember&page=<%= i %>" style="margin: 0 5px; padding: 5px 10px; border: 1px solid #ccc; text-decoration: none; border-radius: 3px;"><%= i %></a>
                <% } %>
            <% } %>

            <% if (currentPage < totalPages) { %>
                <a href="?menu=adminMember&page=<%= currentPage + 1 %>" style="margin: 0 5px; padding: 5px 10px; border: 1px solid #ccc; text-decoration: none; border-radius: 3px;">다음</a>
            <% } else { %>
